...
* Changed done to get the motion editor working in Gazebo robocup3d plugin:
  * In `SendToServer::LaunchMsgLoop()` the following code section is commented because in Gazebo the agent can't be added to the environment while `PlayMode` is set to `PlayOne`:
